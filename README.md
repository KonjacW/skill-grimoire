...
技能**不会自动触发**：宿主在每轮只注入「名字 + 描述 + 路径」，正文要不要读由模型当场决定，**没有任何匹配器**。

本机实测（会话账本快照 2026-09-14；口径 = 账本里**提到该技能 `SKILL.md` 路径的消息数**）：**29 个技能里有 9 个从未被读过一次**；读得最多的三个是 `using-superpowers`(39)、`review-gate`(38)、`subagent-fanout-delivery`(33) —— 它们**正好都在这份路由表里**，而那 9 个从未被读的，**一个都不在表里**。（口径注意：读取次数 ≠ 使用次数，把技能打开来编辑也算一次；账本是活的，数字会随使用增长；此数来自维护者本机，克隆者不可复现。）
