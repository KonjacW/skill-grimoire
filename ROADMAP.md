...
### G17 对话进展汇报的落点与命名尚无定版（2026-09-15 加入技能时登记）

- **背景**：`conversation-progress-report` 于 2026-09-15 进包（口令「开汇报 / 看汇报 / 关汇报」；**纯事件驱动**——只在完成阶段、关键决策、踩坑、口径变更、出实质结果时更新，不设轮数或时间兜底）。它把汇报文件写进维护者的 Obsidian vault（`report/` 目录）；公开副本里该路径按脱敏表替换为 `<OBSIDIAN_VAULT>`。
- **未定版（登记）**：默认命名 `<对话主题>_进展汇报_<YYYYMMDD>.md`（同一对话同一天编辑同一个文件，覆盖式、顶部带更新时间）由维护者**暂定**；技能正文已写明「文件名范式以用户后续规范为准」，故此处不设门禁。
