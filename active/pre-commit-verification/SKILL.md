...
description: "Use when 准备说「做完了」之前：证据强度须与风险相称。"
license: MIT
metadata:
  agent:
...
> 文中的 `spawn_subagent` 指**宿主提供的子代理派发工具**——换成你自己宿主的工具名即可。
...
- After each task when executing a plan (push gate: see `review-gate`)
...
Call `spawn_subagent` directly — it is NOT available inside execute_code or scripts.
...
**风险档 ↔ 最小证据强度对照（免审 / 中风险默认 / 高风险）：** 见 `references/verification-strength-by-risk.md`。
...
- **spawn_subagent returns non-JSON** — retry once with stricter prompt, then treat as FAIL
